...
%% NOTE: sn-jnl defines \orcid with \gdef (it expands in place, storing nothing),
%% so it MUST sit inside the \author* argument. Calling it after \author*
%% emits the ORCID text before \maketitle and yields a near-empty first page.
...
\abstract{We construct a scalar-tensor cosmology in pure induced gravity: a single real scalar field $\Phi$ with a $\Ztwo$-symmetric potential $V_J(\Phi)=\frac{\lambda_0}{4}(\Phi^2-\PhiV^2)^2+V_c$ and non-minimal coupling $\xi\Phi^2R$ replaces the Einstein--Hilbert term, generating the Planck mass via $\vev{\Phi}=\MP/\sqrt{\xi}$. Conformal transformation to the Einstein frame yields a Starobinsky-type slow-roll plateau. With locked $N\equiv\ln(a_{\rm end}/a_*)$ and exact potential slow-roll, $\xi=11.1$ gives $N=50\Rightarrow\lambda_0\simeq6.70\times10^{-8}$, $r\simeq0.00425$, $n_s\simeq0.962$; anomaly $T_{\rm reh}\simeq10^9$ GeV maps to $N\simeq50.7$. Planck $2\sigma$ selects $N\approx45$--$56$ ($r\leq0.0052$; capped at $N_{\max}\simeq55.6$ by instantaneous reheating); LiteBIRD $r>0.01$ would exclude the model class. A Dirac fermion $\psi$ with $-g\Phi\bar\psi\psi$ is a cold-dark-matter \emph{candidate}: $m_\psi=g\PhiV$ shares the same VEV; stability relies on an assumed anomaly-free discrete $\Ztwopsi$ symmetry together with the conformally vanishing $\chi\psi\bar\psi$ vertex that follows from $F=\xi\Phi^2$. Matching $\Omega_{\rm DM}$ with the exact mode-equation spectrum on the exact homogeneous background (the end-of-inflation transition \emph{and} the oscillating-condensate era) selects $g\simeq1.5\times10^{-7}$ ($m_\psi\simeq1.1\times10^{11}$ GeV $\simeq6.6\times10^{-3}H_{\rm inf}$), cold kinematics ($\lambda_{\rm fs}\sim10^{-19}$ Mpc); the absolute normalization is thereby closed at the homogeneous level, validated by a convergence/unitarity/conformal-limit battery, with the transition-shape systematic ($\times1.6$) quoted separately and a 3D preheating cross-check remaining. The potential minimum furnishes a frozen cosmological constant with $w=-1$ (residual-condensate corrections $\leq10^{-10}$); \textbf{this dark-energy sector is calibrated, not predicted}, with $V_c$ fixed by $\Omega_\Lambda=0.683$ and the $10^{110}$ hierarchy unexplained; a confirmed DESI DR2 preference for $w_0\neq-1$ would exclude it and $\Lambda$CDM alike. The identity $\Omega=\Phi/\PhiV$ decouples $\chi$ from $\psi$ at tree level; $m_\chi\sim10^{13}$ GeV suppresses SM fifth forces. Tree-level $\chi\to h\bar h$ and $\chi\to\psi\bar\psi$ are closed; reheating is via conformal anomaly. The SM embedding is RG-stable ($\Delta\xi\leq10^{-6}$); the $w_0=-1$ prediction is shared with $\Lambda$CDM and not independently discriminating; the sharp falsification handles remain the inflationary observables.}
...
\item \textbf{Dark matter:} a Dirac fermion $\psi$, with mass $m_\psi=g\PhiV$ set by the same VEV, is gravitationally produced at the end of inflation and is stable under the discrete $\Ztwopsi$ symmetry together with the conformally vanishing $\chi\psi\bar\psi$ vertex; the condensate itself decays to complete reheating. The relic abundance is fixed at the homogeneous level by the exact mode-equation computation of Sec.~V (Appendix~D0), with a 3D preheating simulation remaining as the non-perturbative cross-check.
...
\paragraph{b. Fermion masses.} Fermions $\psi$ couple through the Yukawa interaction $\mathcal{L}_Y=-g\Phi\bar\psi\psi$, giving $m_\psi=g|\Phi|$. At the minimum, $m_\psi^{(0)}=g\PhiV$. A full Standard Model embedding (including Higgs-mediated masses and RG stability) is constructed in Sec.~VII.
...
\label{eq:matchN}
\end{equation}
with $\Omega_m\simeq0.315$, $\Omega_r=9.15\times10^{-5}$ (the Planck value, $\Omega_r h^2=4.15\times10^{-5}$ with $h=0.674$), $\rho_{\rm reh}=(\pi^2/30)g_*T_{\rm reh}^4$, $\rho_{\rm end}=K_{\rm end}+V_{\rm end}$, $g_{*s,\rm reh}=106.75$, $g_{*s,\rm eq}=3.91$, $g_{*,\rm eq}=3.36$ and $T_{\rm eq}=(30\rho_{\rm eq}/\pi^2g_{*,\rm eq})^{1/4}$. The radiation era is matched by comoving-entropy conservation rather than by the constant-$g_*$ scaling $a\propto\rho^{-1/4}$; the two routes differ by a factor $\simeq2.5$ in $T_{\rm reh}^*$, i.e.\ $\simeq0.30$ $e$-folds in $N$ (the combined effect of entropy matching, the $\rho_{\rm end}=K_{\rm end}+V_{\rm end}$ update and the $\Omega_r$ update; the $g_{*s}$ dilution $106.75\to3.91$ alone would give $(106.75/3.91)^{1/3}\simeq3.0$). At a given $N$, the plateau variable $x_*$ is fixed by the exact potential-slow-roll integral
\begin{equation}
...
\paragraph{Sensitivity table.} Table~\ref{tab:sens} lists exact slow-roll predictions at $\xi=11.1$ under the locked $N$ definition. Planck $2\sigma$ ($n_s\in[0.9565,0.9733]$) allows, on $n_s$ alone, $N\approx45$--$56$ with $r\in[0.0034,0.0052]$; the tighter $1\sigma$ lower edge $n_s\geq0.9607$ selects $N\geq49$. The upper edge is not set by $n_s$ but by reheating: the tabulated $T_{\rm reh}^*(N)$ grows as $e^{3.05N}$, so $T_{\rm reh}^*(56)\simeq7.8\times10^{15}\,$GeV already exceeds the instantaneous-reheating ceiling $T_{\rm reh}^{\rm inst}=(30\rho_{\rm end}/\pi^2g_*)^{1/4}\simeq2.5\times10^{15}\,$GeV obtained at $N=56$; hence $N_{\rm max}\simeq55.6$ and $N=56,57,58$ are physically excluded (Appendix~D).

\paragraph{Two-sided physical window.} The admissible band is bounded on \emph{both} sides by physics, not by the $n_s$ likelihood alone. On the high side, $T^*_{\rm reh}(N)\propto e^{3N}$ while the instantaneous-reheating ceiling falls only as $N^{-1/2}$, giving $N_{\rm max}\simeq55.6$ as shown above. On the low side, the model cannot be pushed to small $N$: the tensor-to-scalar ratio falls to $r=0.01$ at $N_*\simeq32$ in the locked-$N$ convention (the attractor closed form $r\simeq8/(\beta^2N^2)$ gives $N_*\simeq35$), so $r>0.01$ requires $N\lesssim32$, for which the matching condition \eqref{eq:matchN} returns $T_{\rm reh}\simeq2.5\times10^{-16}\,$GeV---more than thirteen orders of magnitude below the BBN floor of $10\,$MeV---so such a point is physically impossible for any reheating mechanism. The viable interval is therefore $N\in[45,55.6]$, and Planck $2\sigma$ on $n_s$ happens to select almost exactly that range. This is what makes the model falsifiable in a definite way: a LiteBIRD-class detection $r>0.01$ would demand $N\lesssim32$ and hence a reheating temperature incompatible with BBN, so it would exclude the model class outright rather than merely disfavour it.
...
52 & $6.21\times10^{-8}$ & 0.9630 & 0.00394 & $1.58\times10^{13}$ & $4.4\times10^{10}$ \\
...
\item \textbf{No renormalizable decay channel.} Being a gauge singlet, $\psi$ has no renormalizable coupling to SM fields; with the $\chi\psi\bar\psi$ vertex identically zero, the only residual interaction is gravitational (which respects $\Ztwopsi$). One precision: a Majorana mass term $\frac12 m_M\psi^c\psi$ is even under $\Ztwopsi$ ($\psi^c\psi\to+\psi^c\psi$) and is therefore \emph{not} excluded by it---$\Ztwopsi$ forbids single-$\psi$ operators (decays), not $\psi$-pair operators. What keeps $\psi$ Dirac rather than Majorana is the UV charge assignment: we treat the absence of an induced Majorana term as part of the anomaly-free UV-completion assumption declared above.
\end{itemize}
...
\label{eq:adiab}
\end{equation}
so adiabaticity fails for $g\lesssim10^{-4}$, and at the abundance-matched value $g\simeq1.5\times10^{-7}$ the parameter \eqref{eq:adiab} reaches $\sim3\times10^{3}$, i.e.\ strongly non-adiabatic $\psi\bar\psi$ pair production---consistent with the light branch found below, which sits at $m_\psi\ll H_{\rm inf}$. This Jordan-frame picture is the \emph{conformal image} of the Einstein-frame gravitational production above, not an independent channel: both are driven by the same single $\Phi(t)$ dynamics through the conformal relation $\tilde g_{\mu\nu}=\Omega^2(\Phi)g_{\mu\nu}$, so they should not be added. The Jordan-frame adiabaticity check serves only as a consistency cross-check on the location of the production peak; the abundance estimate below uses the Einstein-frame result \eqref{eq:npsi} alone. The precise $m_\psi$ that yields the observed abundance is not computable from this parametric estimate alone; it is obtained below from the mode-equation integration, with the oscillating-condensate era included in the exact homogeneous-background computation (Sec.~V) and the remaining non-homogeneous effects parametrically negligible ($\lambda_{\rm self}\sim4\times10^{-11}$, $\rho_\psi/\rho_{\rm end}\sim10^{-19}$), so that a lattice simulation is an independent cross-check rather than the missing ingredient. The same parameter also locates the weakest point of that calculation: at $g\simeq1.5\times10^{-7}$ the quantity \eqref{eq:adiab} is $\sim3\times10^{3}$, some $470$ times larger than at the heavy-branch point \eqref{eq:gwindow}, so the light branch lies deepest in the regime where the post-inflationary mass variation is non-adiabatic and the oscillating condensate---absent from the transition-only calculation---can contribute. A lattice simulation is therefore no longer the only test of the light branch---the controlled-background validation battery of Appendix~D0 (convergence, unitarity, massless/conformal limit, and the exact de Sitter initial-data anchor) now supports it---but it remains the only fully non-perturbative one.
...
\paragraph{One light branch, not two competing branches.} Equation~\eqref{eq:Omegapsi} is non-monotonic in $g$, so the condition $\Omega_\psi=\Omega_{\rm DM}$ does not by itself single out Eq.~\eqref{eq:gwindow}. Because a massless Dirac field is conformally invariant, gravitational production switches off as $m_\psi\to0$; hence $\Omega_\psi(g)\to0$ on the light side and a second solution must exist below the maximum at $g=(2\pi)^{-1}H_{\rm inf}/\PhiV\simeq3.6\times10^{-6}$. Replacing the de Sitter exponential \eqref{eq:npsi} by the actual mode-equation spectrum across the end-of-inflation transition (Appendix~D0), and fixing the model's own anomaly-channel temperature $T_{\rm reh}=2.1\times10^{8}\,$GeV, we find that $\Omega_\psi=\Omega_{\rm DM}$ is met at $g\simeq1.0\times10^{-7}$ in that transition-only calculation, i.e.\ $m_\psi\simeq4.6\times10^{-3}H_{\rm inf}\simeq7.5\times10^{10}\,$GeV, where $n_\psi/H_{\rm inf}^3\simeq1.4\times10^{-5}$ and $m_\psi n_\psi\propto m_\psi^{2}$. Over the same spectrum the heavy side is \emph{not} a competing solution: $\Omega_\psi$ rises monotonically with $m_\psi$ for $m_\psi\gtrsim H_{\rm inf}$ (we measure $\Omega_\psi\approx1.6\times10^{3}$--$3.0\times10^{3}$ over $m_\psi/H_{\rm inf}=3$--$12$), so Eq.~\eqref{eq:gwindow} overproduces $\psi$ by $\sim6\times10^{3}$ and every heavier value overproduces at least as much. The heavy side becomes viable only if $T_{\rm reh}$ is lowered by some four orders of magnitude---we find $T_{\rm reh}\simeq2$--$3\times10^{4}\,$GeV over $m_\psi/H_{\rm inf}\in[1,12]$---which the conformal-anomaly channel of Sec.~IV does not provide. Within the transition calculation the abundance matching is therefore unique. The direction in which the omitted physics moves $g$ is now measured rather than assumed: extending the mode integration through the oscillating-condensate era on the exact homogeneous background (Appendix~D0) raises the production by $R\equiv n_{\rm exact}/n_{\rm avg}\simeq1.24$ at fixed transition, which alone would lower the coupling by $R^{-1/2}\simeq0.90$, but replacing the instantaneous-splice transition by the real slow-roll descent lowers the transition-only production baseline by a factor $\simeq2.7$ and dominates, so the net shift is \emph{upward}: $g\simeq1.5\times10^{-7}$ ($m_\psi\simeq1.1\times10^{11}\,$GeV, $n_\psi/H_{\rm inf}^3\simeq9.5\times10^{-6}$). The transition-only value $g\simeq1.0\times10^{-7}$ is thus a \emph{lower}, not an upper, reference point of the final coupling. Equation~\eqref{eq:gwindow} is retained as the reference point of the exponential framework rather than as a prediction.

The mechanism is viable and matches the observed abundance on the light branch at $g\simeq1.5\times10^{-7}$ in the exact homogeneous-background computation (transition + oscillating-condensate era, validated by the convergence/unitarity/conformal battery of Appendix~D0). In that treatment the split of the condensate energy between radiation and the stable $\psi$ component requires no additional input: $\psi$ is a test field with $\rho_\psi/\rho_{\rm end}\sim10^{-19}$ at the matching point, so the two channels are independent; a 3D Floquet/lattice simulation remains as the non-perturbative cross-check (Discussion).

\paragraph{Robustness window, and what remains beyond the homogeneous computation.}
The matching point is a two-parameter statement rather than a number. On the light branch $m_\psi n_\psi\propto m_\psi^{2}$, hence $g\propto T_{\rm reh}^{-1/2}$: a shift in the reheating temperature moves $g$ and $m_\psi$ but leaves the branch itself intact. Table~\ref{tab:trehband} propagates a factor-$10^{2}$ band in $T_{\rm reh}$---spanning the physical-$\alpha_s$ anomaly value, the fiducial $10^{9}$~GeV reference, and the possible one-to-two-order non-perturbative enhancement---through the locked-$N$ matching of Sec.~III. Across that band $N$ moves by $1.5$, $n_s$ by $1.1\times10^{-3}$ (a few per cent of the Planck uncertainty) and $r$ by $5.5\%$, so the inflationary predictions are \emph{not} hostage to the reheating history. The dark-matter matching point, by contrast, moves by a full factor of $10$ in $g$ and $m_\psi$ (through $m_\psi=g\PhiV$), while the free-streaming length remains $17$--$18$ orders of magnitude below the warm-dark-matter bound throughout: the light branch stays cold dark matter over the whole band. Note that $\lambda_{\rm fs}$ scales approximately as $m_\psi^{-1/2}$ across the table rather than as $m_\psi^{-1}$ (measured exponent $-0.54$ to $-0.55$): the length is accumulated while the particle is relativistic during matter-like condensate domination, so it scales as $\sqrt{a_{\rm NR}}$ with $a_{\rm NR}\propto1/m_\psi$, and the full three-era expansion history---not an instantaneous non-relativistic estimate---enters the integral. The oscillating-condensate contribution, now measured on the exact homogeneous background, indeed raises the production ($R=n_{\rm exact}/n_{\rm avg}\simeq1.24$ at the matching point) and lowers the matching $g$ by $R^{-1/2}\simeq0.90$---the branch shifts, it does not fail. But the net shift is set by a different systematic: replacing the instantaneous-splice transition by the real slow-roll descent lowers the transition-only production baseline by a factor $\simeq2.7$, which dominates and moves the matched coupling to $g\simeq1.5\times10^{-7}$ (Sec.~V). Quoting the transition-only baseline $g\simeq1.0\times10^{-7}$ is an abbreviation for a window: over the range $T_{\rm reh}=10^{8}$--$10^{9}$~GeV spanned by the anomaly value and the fiducial reference it is $g\simeq(0.47$--$1.5)\times10^{-7}$ and $m_\psi\simeq3.4$--$11\times10^{10}$~GeV, widening to $g\simeq1.5\times10^{-8}$ and $m_\psi\simeq1.1\times10^{10}$~GeV if the two-order non-perturbative enhancement of the last entry is realized; the exact-background computation of Appendix~D0 multiplies this entire window by $1.48$ (the $T_{\rm reh}^{-1/2}$ scaling is unchanged), giving $g\simeq(0.7$--$2.2)\times10^{-7}$ and $m_\psi\simeq(5.0$--$16)\times10^{10}$~GeV across the window; at the model's own $T_{\rm reh}=2.1\times10^{8}$~GeV the matched coupling is $g\simeq1.5\times10^{-7}$ ($m_\psi\simeq1.1\times10^{11}$~GeV).

Concretely, the computation needed to close the loop is the same action with $m_\chi/H_{\rm inf}\simeq1.99$, $\xi=11.1$, $\lambda_0\simeq6.70\times10^{-8}$ and $g\sim10^{-7}$, evolved from the end of slow roll through the condensate phase, with two required outputs: the absolute normalization of $n_\psi$ including the oscillating-condensate contribution, and the partition of the condensate energy between radiation and the stable $\psi$ component (Discussion~\#3). The first output is now supplied by the exact homogeneous-background mode integration of Appendix~D0 ($g\simeq1.5\times10^{-7}$ at the model's own $T_{\rm reh}$, validated by the convergence/unitarity/conformal battery). The second requires no additional computation in the regime established there: $\psi$ is a test field with $\rho_\psi/\rho_{\rm end}\sim10^{-19}$ at the matching point, so $\psi$ production does not compete with reheating and the partition is fixed by the independent anomaly channel. Two features made the homogeneous computation non-standard but bounded: the condensate fragments inefficiently ($\lambda_{\rm self}\sim4\times10^{-11}$), so the homogeneous background is a sound starting point; and the fermion coupling is large enough---the adiabaticity parameter \eqref{eq:adiab} being $\sim3\times10^{3}$---that no perturbative treatment of $\psi$ production is adequate (the Bogoliubov integration of the mode equation is used instead). What a 3D lattice simulation would still decide is whether the non-linear preheating dynamics---spatial fragmentation, resonance, backreaction of the produced particles---invalidates the homogeneous starting point; parametrically it should not, and the fermion remains the one sector of this framework for which such a non-perturbative cross-check is worth having.

\begin{table}[t]
\centering
\caption{Propagation of the reheating-temperature uncertainty through the locked-$N$ matching and the light-branch abundance relation; computed by \texttt{scripts/treh\_error\_band.py} on the transition-only spectrum (the instantaneous-splice baseline). The inflationary observables are insensitive to $T_{\rm reh}$, whereas the dark-matter matching point is not and is best quoted as a window. The second row is the model's own physical-$\alpha_s$ anomaly value. Free-streaming lengths use the diagnostic of this section. The exact homogeneous-background computation of Appendix~D0 multiplies the quoted $g$ and $m_\psi$ columns uniformly by $1.48$, leaving the $T_{\rm reh}^{-1/2}$ scaling and the free-streaming conclusion unchanged.}
\label{tab:trehband}
...
$2.1\times10^{8}$ & 50.23 & 0.9618 & 0.00421 & $1.0\times10^{-7}$ & $7.5\times10^{10}$ & $9.5\times10^{-20}$ \\
$1.0\times10^{9}$ & 50.74 & 0.9621 & 0.00413 & $4.7\times10^{-8}$ & $3.4\times10^{10}$ & $1.4\times10^{-19}$ \\
$1.0\times10^{10}$ & 51.51 & 0.9627 & 0.00402 & $1.5\times10^{-8}$ & $1.1\times10^{10}$ & $2.7\times10^{-19}$ \\
\end{tabular}
\end{table}
...
\begin{equation}
V_c=\Omega_\Lambda\cdot3H_0^2\MP^2\approx2.5\times10^{-47}\,{\rm GeV}^4.
\label{eq:Vc}
\end{equation}
The $10^{110}$ hierarchy between $V_0\sim10^{63}\,$GeV$^4$ and $V_c$ is spanned by the conformal factor $e^{-4\varphi}$ but \emph{not} dynamically explained; this is the cosmological constant problem, shared with all $\Lambda$CDM models. This model reproduces a late-time cosmological constant with $w=-1$ (up to corrections $\leq10^{-10}$ for $T_{\rm reh}\geq1\,$GeV; see App.~D5), but does not address the core fine-tuning puzzles. Two distinct tuning problems remain: (i) why $\PhiV=\MP/\sqrt{\xi}\sim10^{17}\,$GeV sets the Planck/electroweak hierarchy---the VEV is a fixed point of the potential by construction but its magnitude is not dynamically explained; (ii) why $V_c\sim10^{-47}\,$GeV$^4$ is $10^{110}$ times smaller than the inflationary scale $V_0\sim10^{63}\,$GeV$^4$---the conformal factor $e^{-4\varphi}$ maps $V_c$ from the Jordan to the Einstein frame but the tiny value of $V_c$ itself is not derived from a symmetry or dynamical principle~\cite{Weinberg1989}. Moreover, the heavy fields $\chi$ ($m_\chi\sim10^{13}\,$GeV) and $\psi$ ($m_\psi\simeq1.1\times10^{11}\,$GeV) contribute radiative corrections $\delta V_c\sim m^4/(16\pi^2)\sim10^{42}$--$10^{52}\,$GeV$^4$, exacerbating the fine-tuning by an additional $\sim10^{89}$--$10^{98}$ orders relative to the observed $V_c$; this makes the cosmological constant problem quantitatively more severe in this model than in the Standard Model alone, an issue shared with any theory containing heavy scalar or fermion degrees of freedom and deferred to a UV completion. $V_c$ is an empirical observational input, not dynamically derived. During reheating, the minimum at $\Phi=\PhiV$ is protected against thermal and curvature perturbations: the field mass $m_\Phi\sim2.7\times10^{14}\,$GeV exceeds the reheating temperature $T_{\rm reh}\sim10^9\,$GeV by five orders of magnitude, thermal mass corrections $\delta m_\Phi^2\sim g^2 T^2$ are $\leq10^{-20}m_\Phi^2$, and curvature-driven displacements $\sim\xi\PhiV R/m_\Phi^2$ are $\leq10^{-13}$; the minimum remains stable throughout all post-inflationary epochs. A detection of $w_0\neq-1$ would exclude this model and $\Lambda$CDM alike, and the latest data lean that way: DESI DR2, combined with the CMB, prefers time-evolving dark energy over $\Lambda$CDM at $3.1\sigma$ ($2.8$--$4.2\sigma$ once supernovae are included), in the quadrant $w_0>-1$, $w_a<0$~\cite{DESI2025}. The present model has no degree of freedom that can produce such an evolution---$\chi$ is frozen and $V_c$ is a constant---so its dark-energy sector is excluded if that preference survives as a physical effect rather than as a systematic. Because $\Lambda$CDM predicts the same $w_0=-1$, the test bears on the cosmological-constant hypothesis rather than on the unification this framework proposes.

\subsection{Drift of the fundamental constants}

The same residual Ricci-driven displacement that enters the $\Delta w$ budget above also drives a drift of the fundamental constants, providing a consistency check that the induced-gravity framework generates no observable violation of the equivalence principle or time-variation of couplings. All effects derived here lie $>90$ orders below current experimental sensitivity; this subsection may be skipped on a first reading without loss of continuity.
...
\begin{itemize}
\item \textbf{Gauge sector:} conformally invariant at tree level; conformal-anomaly-induced $\chi$--gauge coupling at 1-loop (discussed in Secs.~IV and VII) does not affect the $T_{\rm reh}$ or fifth-force conclusions.
...
\begin{enumerate}
\item \textbf{Inflationary observables (locked $N$):} exact potential slow roll with $N=\ln(a_{\rm end}/a_*)$. A measurement $r>0.01$ at 95\% CL by LiteBIRD~\cite{LiteBIRD2023} excludes the model: Planck $2\sigma$ allows $N\approx45$--$56$ where $r\leq0.0052$ (the statistical $n_s$ band, capped at $N_{\max}\simeq55.6$ by instantaneous reheating; Table~\ref{tab:sens}).
\item \textbf{Dark energy equation of state:} $w_0=-1$ (up to residual-condensate corrections $\leq10^{-10}$ for $T_{\rm reh}\geq1\,$GeV; App.~D5). DESI DR2 already reports a $3.1\sigma$ preference for $w_0\neq-1$~\cite{DESI2025}, which this model cannot accommodate (the field is frozen and $V_c$ is a constant); but because $\Lambda$CDM predicts the same value, such a detection would exclude both rather than discriminate between them.
...
\paragraph{b. Level 2: Dark matter prediction (quantitative matching; normalization from the homogeneous mode computation, lattice cross-check open).}
\begin{enumerate}\setcounter{enumi}{4}
\item The dark matter is a Dirac fermion $\psi$ whose abundance is matched to $\Omega_{\rm DM}$ at $g\simeq1.5\times10^{-7}$, i.e.\ $m_\psi\simeq6.6\times10^{-3}H_{\rm inf}\simeq1.1\times10^{11}\,$GeV, stable under the discrete $\Ztwopsi$ symmetry (assuming an anomaly-free UV gauge completion) and the conformally vanishing $\chi\psi\bar\psi$ vertex, and produced gravitationally (non-adiabatic metric change at the end of inflation, in the Einstein frame). The absolute normalization is obtained from the exact homogeneous-background mode computation (transition + oscillating-condensate era, Appendix~D0); a full Floquet/lattice analysis remains as the non-perturbative cross-check.
\end{enumerate}
...
\item \textbf{Dark matter: stability and relic abundance.} The dark matter is the Dirac fermion $\psi$, stable under the discrete $\Ztwopsi$ symmetry (conditional on UV gauge completion) plus the conformally vanishing $\chi\psi\bar\psi$ vertex (Sec.~V). Its relic abundance is set by gravitational particle production at the end of inflation---the non-adiabatic change of the Einstein-frame metric, which in the Jordan frame appears as the time-varying mass $m_\psi(t)=g|\Phi(t)|$ (a single physical process, not two additive channels). A quantitative $\Omega_\psi$ normalization is obtained from the exact homogeneous-background mode integration (Sec.~V, Appendix~D0): the abundance matching sits on the light branch at $m_\psi\simeq6.6\times10^{-3}H_{\rm inf}\simeq1.1\times10^{11}\,$GeV ($g\simeq1.5\times10^{-7}$), the dominant residual uncertainty being the transition-shape systematic (factor $\simeq1.6$) quoted there; a dedicated Floquet/lattice simulation remains the non-perturbative cross-check of the homogeneous treatment.

\item \textbf{Preheating energy partition.} The gravitational $\psi$ production and the condensate decay through conformal-anomaly channel compete for the condensate energy. If the $\psi$ channel is too efficient, an excessive fraction of the condensate energy is diverted into stable matter rather than radiation, which could suppress the reheating temperature. In the homogeneous treatment the split is already determined: $\psi$ is a test field with $\rho_\psi/\rho_{\rm end}\sim10^{-19}$ at the matching point (Appendix~D0), so the radiation/$\psi$ channels are independent and the partition is set by the anomaly decay; a lattice simulation would verify that this test-field hierarchy survives the full 3D preheating dynamics.

\item \textbf{Stochastic gravitational-wave background from preheating.} The oscillating condensate and any non-perturbative field inhomogeneities during preheating generically source a stochastic gravitational-wave background. For the present model, the extremely weak coupling of $\chi$ to SM fields ($g_{\chi ff}\sim m_f/\MP\leq10^{-17}$) and the closure of the $\chi\to h\bar h$ channel suppress parametric resonance, so the inhomogeneous component of the condensate is expected to be small and the resulting GW background correspondingly weak. A rough estimate using the standard scaling $h^2\Omega_{\rm GW}\sim10^{-5}(H_{\rm reh}/\MP)^2$ with $H_{\rm reh}\simeq\sqrt{\pi^2 g_*/90}\,T_{\rm reh}^2/\MP\sim1.4\,$GeV (corresponding to $T_{\rm reh}\sim10^9\,$GeV) gives $h^2\Omega_{\rm GW}\sim3\times10^{-42}$, some 30 orders below the sensitivity of LISA ($\sim10^{-12}$) and pulsar timing arrays ($\sim10^{-10}$). A dedicated lattice computation of the GW spectrum is deferred to the same future preheating simulation that would verify the homogeneous $\Omega_\psi$ determination of Sec.~V.
...
\item \textbf{Observational indistinguishability from $\Lambda$CDM.} The dark-energy prediction $w=-1$ (up to $\leq10^{-10}$ corrections) and the $\Lambda$CDM-identical structure-growth prediction (Sec.~VI) imply that the late-time model is observationally indistinguishable from $\Lambda$CDM by any foreseeable survey. The testable content is therefore confined to the inflationary sector ($n_s,r,n_t,f_{\rm NL}$) and the dark-matter relic abundance (normalization closed at the homogeneous level, Sec.~V and Appendix~D0; the 3D lattice cross-check remains); the dark-energy sector, while structurally unified with inflation, does not provide independent falsifiable signatures. Within the inflationary sector, moreover, $n_s(N)$ and $r(N)$ coincide numerically with the $R^2$ (Starobinsky) prediction at the same $N$ (Sec.~I), so an observed $(n_s,r)$ pair fixes $N$ without discriminating between the two models, and a LiteBIRD detection of $r>0.01$ excludes the two together. The one genuinely non-degenerate relation of this framework is the dark-matter one, through the fixed relation \eqref{eq:mpsiH}: should a UV completion or any future non-gravitational effect ever fix $g$, Eq.~\eqref{eq:mpsiH} would translate it into a prediction for $m_\psi$ at fixed $\xi$, and conversely---an internal consistency relation today, since $\psi$ couples only gravitationally (Sec.~V). This is the strongest discriminating statement the present version of the model supports, and it is exactly why the dark-matter normalization of Sec.~V matters more here than in a model whose inflationary sector is already distinctive.

\item \textbf{Field-space structure at $\Phi=0$.} The Jordan-frame kinetic metric $g_{\Phi\Phi}=\xi\Phi^2$ degenerates at $\Phi=0$, rendering this point a boundary of the classical field space. Consequently, classical field configurations cannot cross $\Phi=0$; the two $\Ztwo$ vacua are topologically separated at the classical level. The thin-shell analysis of Sec.~VIII confirms that the EFT itself signals its breakdown at $\Phi=0$ through the matching-condition degeneracy, motivating a quantum-tunneling (instanton) description of any inter-vacuum transitions.

\item \textbf{Higher-order corrections to the dark-energy equation of state.} The $w=-1$ result (Sec.~VI) follows from the quadratic expansion of $V_E(\varphi)$ near $\varphi=0$. The quartic term $\sim\lambda_0\PhiV^4\varphi^4/\MP^4$ contributes a fractional correction $\Delta w/w\sim(\lambda_0/\xi^2)\varphi^2\simeq5.4\times10^{-10}\varphi^2$. For the frozen field value $\varphi\sim\delta\Phi/\MP\sim(H_0/m_\chi)^2\sim2\times10^{-111}$ (the Ricci driving of Sec.~VI), this correction is $\sim2\times10^{-231}$, utterly negligible. The harmonic approximation is therefore exact for all practical purposes.
...
\item \textbf{Thermalization after reheating.} The reheating analysis assumes that decay products thermalize rapidly, establishing a well-defined temperature $T_{\rm reh}$. At $T\sim10^9\,$GeV, the SM gauge interaction rate is $\Gamma_{\rm therm}\sim\alpha_s^2 T\simeq1.4\times10^{6}\,$GeV (one-loop running from the Sec.~IV anchor $\alpha_s(m_Z)=0.1179$ gives $\alpha_s(10^{9}\,{\rm GeV})\simeq0.038$), while the Hubble rate is $H\simeq\sqrt{\pi^2 g_*/90}\,T^2/\MP\sim1.4\,$GeV; the ratio $\Gamma_{\rm therm}/H\sim10^{6}$ still confirms near-instantaneous thermalization. Gravitationally produced particles are initially non-thermal, but their energy density is $\sim10^{13}$ times smaller than the anomaly-driven radiation bath (Sec.~IV), so they thermalize with the dominant component within a few interaction times, well before BBN.
...
\item \textbf{Additional consistency checks.} Several further consistency conditions are satisfied parametrically: (i) \emph{Dark matter self-interactions:} $\psi$--$\psi$ scattering via graviton exchange gives $\sigma_{\psi\psi}/m_\psi\sim G_N^2 m_\psi\sim1\times10^{-69}\,{\rm cm}^2/{\rm g}$, $\sim69$ orders below the bullet-cluster bound ($\leq1\,{\rm cm}^2/{\rm g}$); loop-induced scalar exchange is further suppressed by $g_{\chi\psi\psi}^{\rm eff}\sim10^{-11}g$. (ii) \emph{Tremaine--Gunn bound:} the non-thermal $\psi$ phase-space density $Q\equiv\rho/\sigma_v^3$ at production (with velocity dispersion $\sigma_v\sim H_{\rm inf}/m_\psi$) is $Q\sim m_\psi^4\sim1.4\times10^{44}\,$GeV$^4$, far above the dwarf-galaxy lower limit. (iii) \emph{Graviton dark radiation:} relativistic gravitons produced during reheating contribute $\Delta N_{\rm eff}\leq10^{-5}$, negligible compared to Planck. (iv) \emph{Entropy production:} irreversible; total entropy per comoving volume increases by $\sim10^{13}$ during reheating. (v) \emph{Neutrino sector:} $\chi$--$\nu$ effective coupling $\sim m_\nu/\MP\leq10^{-27}$, no observable consequences. (vi) \emph{Primordial magnetic fields:} anomaly-induced seed after $a^{-2}$ redshift gives $B_0\sim10^{-25}\,$G, far below blazar lower bounds. (vii) \emph{Geodesic deviation:} Einstein-frame geodesic equation reproduces Yukawa suppression for SM fermions. (viii) \emph{Gravitational production of $\psi$:} the abundance is obtained from the Bogoliubov coefficients of the mode equation across the end-of-inflation transition and the oscillating-condensate era, not from an instantaneous-pulse approximation; the homogeneous computation is validated by the convergence/unitarity/conformal-limit battery of Appendix~D0, and a full Floquet/lattice treatment remains as the 3D cross-check (Discussion). (ix) \emph{Particle production backreaction:} $\rho_{\rm part}\leq10^{-13}\rho_\chi$. (x) \emph{Late-time scalar--photon coupling:} mixing angle $\sim10^{-55}$, CMB distortions $\leq10^{-60}$. (xi) \emph{Ghost freedom:} Einstein-frame theory is GR plus a minimally coupled canonical scalar; kinetic terms positive-definite. (xii) \emph{Cosmic strings from $U(1)_D$ breaking:} inflated away or never form within horizon. (xiii) \emph{Jordan-frame PPN cross-check:} $|\gamma_J-1|\leq10^{-5}$. (xiv) \emph{Spectral index running:} $\alpha_s\simeq-2/N^2\sim-8\times10^{-4}$, consistent with Planck. (xv) \emph{CMB B-mode spectrum:} peaks at $\ell\sim80$ for $r\simeq0.005$, within LiteBIRD reach. (xvi) \emph{Domain-wall observational null:} $e^{-3N}\sim10^{-66}$ renders all wall observables negligible.
\end{enumerate}
...
\begin{itemize}
\item $N$ is defined as $N=\ln(a_{\rm end}/a_*)$ with explicit matching assumptions \eqref{eq:matchN}. Planck $2\sigma$ selects $N\approx45$--$56$ (the statistical $n_s$ band; instantaneous reheating caps the physical window at $N_{\max}\simeq55.6$); anomaly-dominated $T_{\rm reh}\sim10^9\,$GeV maps to $N\simeq50.7$ in this convention (self-consistent $T_{\rm reh}^*$ for $N=50$ is $\simeq1.1\times10^{8}\,$GeV).
...
\item The dark-matter mass is fixed by the abundance matching at $m_\psi\simeq6.6\times10^{-3}H_{\rm inf}\simeq1.1\times10^{11}\,$GeV ($g\simeq1.5\times10^{-7}$) and follows from the gravitational-production mechanism; the absolute normalization is obtained from the exact homogeneous-background mode computation (Appendix~D0), with a Floquet/lattice simulation as the remaining cross-check.
\end{itemize}
...
\item Non-perturbative (3D lattice) confirmation of the homogeneous preheating treatment: the absolute normalization of the dark-matter relic abundance is obtained at the homogeneous level (Sec.~V, Appendix~D0), and the preheating energy partition is fixed there by the test-field hierarchy $\rho_\psi/\rho_{\rm end}\sim10^{-19}$; what remains for the lattice is the 3D dynamics of fragmentation and resonance.
...
The key observational test is $r$ under the locked $N$ definition: Planck $2\sigma$ allows $N\approx45$--$56$ with $r\leq0.0052$ (the statistical $n_s$ band, capped at $N_{\max}\simeq55.6$ by instantaneous reheating; Table~\ref{tab:sens}). A LiteBIRD-class measurement $r>0.01$ at 95\% CL would exclude the model class in this convention. The frozen $w_0=-1$ prediction is shared with $\Lambda$CDM, so it discriminates neither model from the other; it is nonetheless a live exposure, since DESI DR2 already prefers $w_0\neq-1$ at $3.1\sigma$~\cite{DESI2025}, and a confirmation would exclude this dark-energy sector (though not the inflationary one). The inflationary observables $n_s$, $r$, $n_t$ and $f_{\rm NL}$ are the sharpest handles for \emph{excluding} the model class, but at a given $N$ they are numerically identical to the $R^2$ (Starobinsky) predictions and therefore do not discriminate between the two; the one non-degenerate relation of the framework is the dark-matter relation \eqref{eq:mpsiH}, whose observational content is fixed at the homogeneous level by the normalization of Sec.~V ($g\simeq1.5\times10^{-7}$, with the transition-shape systematic quoted there), pending only the non-perturbative cross-check.

\paragraph{Calibration vs.\ prediction (summary).} Rigid predictions of the action at chosen $(\xi,N)$ are the inflationary functions $n_s(N)$ and $r(N)$ and the derived scales $H_{\rm inf}$, $m_\chi$, $U^{1/4}=(3\MP^2H_{\rm inf}^2)^{1/4}$. Calibrated or a-posteriori inputs are: $\Omega_\Lambda$ (hence $V_c$), $\lambda_0$ (from $A_s$), $\xi$ (within the $r$ bound), and the dark-matter Yukawa $g$ (from $\Omega_{\rm DM}$; matched at $g\simeq1.5\times10^{-7}$ on the light branch by the exact homogeneous-background computation of Appendix~D0, with the lattice cross-check open). Dark energy is structurally present in the action but observationally degenerate with $\Lambda$CDM; the hard falsification handle lives in the inflationary sector.
...
#   N=51 -> lam0=6.45e-08, ns=0.9623, r=0.00409;  T_reh*(N=51)=2.2e9 GeV (Table I)
# H = sqrt(lam0)*M_P/(2*sqrt(3)*xi)
\end{verbatim}}

\paragraph{Key results (locked $N$, $\xi=11.1$).} At $N=50$: $\lambda_0=6.70\times10^{-8}$, $n_s=0.9616$, $r=0.00425$, $V_0\simeq4.78\times10^{63}\,$GeV$^4$, $H_{\rm inf}\simeq1.64\times10^{13}\,$GeV, $m_\chi\simeq3.25\times10^{13}\,$GeV, $U^{1/4}\simeq8.3\times10^{15}\,$GeV. At $N=51$ ($T_{\rm reh}^*=2.2\times10^{9}\,$GeV, Table~\ref{tab:sens}): $\lambda_0=6.45\times10^{-8}$, $n_s=0.9623$, $r=0.00409$, $H_{\rm inf}\simeq1.61\times10^{13}\,$GeV; the anomaly value $T_{\rm reh}\sim10^9\,$GeV maps instead to the interpolated $N\simeq50.7$ (Sec.~III). In all cases $U^{1/4}=(3\MP^2H_{\rm inf}^2)^{1/4}$.
...
\paragraph{Reheating.} The vanishing vertex means no perturbative $\chi\to\tilde\psi\tilde\psi$ decay, for any $g$. For $m_\psi\gtrsim m_\chi/2$ the decay is additionally kinematically forbidden; at the abundance-matched value $g\simeq1.5\times10^{-7}$ of Sec.~V the kinematic closure no longer holds but the vertex closure still does. The time-varying Jordan-frame mass $m_\psi=g|\Phi|$ is adiabatic for $g\gtrsim10^{-4}$ and non-adiabatic at the dark-matter value $g\simeq1.5\times10^{-7}$; this is the Jordan-frame image of the Einstein-frame gravitational production that sources the dark matter of Sec.~V (a single physical process, not an additive channel), and it does not use the perturbative vertex. Reheating proceeds through conformal-anomaly production together with condensate decay (the gravitational pulse provides only a seed bath and cannot reheat on its own); under the locked $N$ definition this allows $N\simeq45$--$56$ by Planck $2\sigma$ (capped at $N_{\max}\simeq55.6$; Sec.~III), with anomaly $T_{\rm reh}\sim10^9\,$GeV at $N\simeq50.7$ (Sec.~III, Table~\ref{tab:sens}).
...
\paragraph{0. Numerical validation of the end-of-inflation state.} The quantities used in Secs.~III--V are obtained by integrating the exact Klein--Gordon equation in the Einstein frame, $\ddot\chi+3H\dot\chi+V_E'(\chi)=0$ with $3\MP^2H^2=\frac12\dot\chi^2+V_E$, rather than by evaluating slow-roll closed forms. Two conventions matter and are stated explicitly here. (i) In $e$-fold variables the Friedmann constraint reads $H^2=V_E/(3-p^2/2)$ with $p\equiv d\chi/dN$, which degenerates as $V_E\to0$; we therefore integrate the $e$-fold system only over the slow-roll regime and switch to cosmic time for the oscillatory phase. (ii) The end of inflation is defined by $\epsilon_V=1$, where slow roll has already broken down; the slow-roll velocity $\dot\chi=-V_E'/(3H)$ must \emph{not} be imposed there, since it would force $K_{\rm end}=V_{\rm end}/3$ and $\epsilon_H=3/4$ by construction. Integrating instead from $\epsilon_V=0.05$ up to $\epsilon_V=1$ gives $x_{\rm end}=0.76367$, $\epsilon_H(x_{\rm end})=0.4988$, $V_{\rm end}/V_0=0.28520$, $K_{\rm end}/V_{\rm end}=0.1994$, and $\rho_{\rm end}/V_{\rm end}=1.1994$. These reproduce the locked-$N$ fiducial values quoted in Sec.~III and give the barrier ratio $K_{\rm end}/\Delta V_J\simeq0.057$ of Sec.~III. The same framework extends through the oscillating-condensate era: the mode equation is integrated on the exact homogeneous background (real slow-roll descent, numerically resolved condensate oscillations with $\langle w\rangle\simeq-0.02$ over the first $e$-folds, and an analytic matter-domination extension), with controlled backgrounds isolating the two systematics. The oscillating condensate raises the production by $R\equiv n_{\rm exact}/n_{\rm avg}\simeq1.24$ at the matching point, while replacing the instantaneous-splice transition by the real descent lowers the transition-only production baseline by a factor $\simeq2.7$ (a factor $\simeq1.6$ in the matched $g$, since $g\propto n_\psi^{-1/2}$ on the light branch); the net effect moves the matched coupling from $g\simeq1.0\times10^{-7}$ to $g\simeq1.5\times10^{-7}$ ($m_\psi\simeq1.1\times10^{11}\,$GeV at $\Omega_\psi=0.265$, the model's own $T_{\rm reh}=2.1\times10^{8}\,$GeV). The computation is validated against the published transition-only spectrum (reproduced at the $10^{-9}$ level on the shared $p=2$ background), the massless/conformal limit (exactly zero production), unitarity ($\bigl||\alpha|^2+|\beta|^2-1\bigr|\lesssim10^{-12}$), and a convergence battery in the phase step, $k$ grid, starting $e$-folds, freeze criterion and integration endpoint. The same integration supplies the reheating ceiling used in Sec.~III: $T_{\rm reh}^{\rm inst}(N)=(30\rho_{\rm end}(N)/\pi^2g_*)^{1/4}$ decreases only as $N^{-1/2}$, whereas the tabulated $T^*_{\rm reh}(N)$ required by cosmological matching grows as $e^{3.05N}$; the two intersect at $N_{\rm max}=55.6$ (or $55.9$ if the naive ceiling $V_{\rm end}^{1/4}$ is used instead). This is the reason the $n_s$-allowed band is quoted as $N\approx45$--$56$: with $N_{\rm max}=55.6$, the points $N=56$, $57$ and $58$ would require a reheating temperature above instantaneous reheating. The numerical results quoted above and in Secs.~III--VI were obtained with the analysis scripts accompanying this work: \texttt{background\_and\_reheating.py} (exact Klein--Gordon integration, reheating channels and the $N$ window), \texttt{psi\_production\_bogoliubov.py} (de Sitter Bogoliubov index and the abundance matching), \texttt{psi\_abundance\_oscillating.py} (mode-equation abundance across the end-of-inflation transition), \texttt{dm\_gap\_closure\_test.py} (abundance matching on the light branch with the exact mode-equation spectrum across the end-of-inflation transition, and the free-streaming check), \texttt{psi\_mode\_oscillating.py} (exact-background mode integration through the oscillating-condensate era: controlled backgrounds, validation battery and the final matching of Sec.~V), \texttt{treh\_error\_band.py} (propagation of the reheating-temperature uncertainty to $(n_s,r)$ and to the dark-matter matching window), \texttt{residual\_quintessence.py} (two-fluid Boltzmann integration and the $\Delta w$ budget), and \texttt{lock\_n\_convention.py} (the locked-$N$ grid behind Table~\ref{tab:sens} and the figures), on the shared constants and the independent action-level derivation of \texttt{cosmo\_model.py} and \texttt{derive\_from\_action.py} and on the closed forms behind the order-of-magnitude claims in \texttt{order\_estimates.py}, together with \texttt{verify\_numerics.py}, \texttt{consistency\_checks.py}, \texttt{audit\_tex\_numbers.py} and \texttt{audit\_readme\_numbers.py} (audits that recompute every quantitative claim printed here from the current scripts, check every table cell against the script that produces it, and scan for superseded values). All are publicly available at \url{https://github.com/huyamingc/induced-gravity-cosmology}, together with the \LaTeX{} source of this manuscript and a single entry point, \texttt{scripts/run\_all.py}, that regenerates the numerical results and figures quoted in this work.
...
\label{eq:D5}
\end{equation}
where $\rho_{\rm DM}^{(0)}=\Omega_{\rm DM}\rho_{\rm crit}^{(0)}\approx9.7\times10^{-48}\,$GeV$^4$ and $T_0\approx2.36\times10^{-13}\,$GeV. For $T_{\rm reh}\in[10^{-2},10^{15}]\,$GeV this gives $\rho_{\rm cond}(a_{\rm reh})<(5.2\times10^{-16}\text{--}5.2\times10^{35})\,$GeV$^4$. Comparing with the initial condensate energy density $\rho_{\rm cond}(a_{\rm end})\sim\frac12 m_\chi^2\MP^2\simeq3\times10^{63}\,$GeV$^4$, the required suppression factor is $\sim4\times10^{27}$--$4\times10^{78}$ (the upper end corresponding to $T_{\rm reh}$ at the BBN floor). This suppression is supplied by the decay of the condensate (channel (iv) of Sec.~IV): because $\Gamma t_0\gg1$ for any $T_{\rm reh}\gtrsim10^{-2}\,$GeV, the residual condensate at $a_0$ is strictly zero (D5c), and the locked $N$ is confined to the Planck-compatible band (anomaly $T_{\rm reh}\sim10^9\,$GeV $\Rightarrow$ $N\simeq50.7$).
...
\begin{equation}
\rho_{\rm cond}(a_0)<\rho_{\rm DM}^{(0)}\approx9.7\times10^{-48}\,{\rm GeV}^4,
\label{eq:D12}
\end{equation}
giving the constraint $\rho_{\rm cond}(a_0)/V_c\leq\Omega_{\rm DM}/\Omega_\Lambda\sim0.39$. This is a \emph{requirement}, not a computed value: an absolutely stable condensate would give $\rho_{\rm cond}(a_0)/V_c\simeq7\times10^{17}$ and would overclose the universe by $\sim10^{18}$. The requirement is met because the condensate decays: the decay-completeness condition $\Gamma t_0\gg1$ holds by $\geq20$ orders of magnitude for any $T_{\rm reh}\gtrsim10^{-2}\,$GeV, so the residual at $a_0$ is strictly zero (see App.~D5c). This bounds the residual condensate's contribution to the dark-energy equation of state; combined with the rapid damping (Eq.~\eqref{eq:D7}) it confirms that the condensate is cosmologically irrelevant at late times, as asserted in Sec.~VI.
...
\end{aligned}
\end{equation}
the third line vanishes because $\Gamma t_0\gg1$ for any $T_{\rm reh}\gtrsim10^{-2}\,$GeV (see App.~D5c), so the condensate tail contributes to $\Omega_m$ but not to $\Delta w_{\rm DE}$; and $\Delta\mathcal{A}/\mathcal{A}=+\xi R/(\lambda_0\PhiV^2)$.
...
\paragraph{Use of AI-assisted tools.} Large language model tools were used to assist with (i) writing and refactoring the Python analysis scripts that reproduce the numerical results, (ii) independently recomputing and cross-checking numerical values quoted in the text, and (iii) language editing and mechanical consistency checks on cross-references, figure inclusion, and the bibliography. Every AI-assisted output was treated as a proposal and was verified by the author against the model's own equations before being accepted; the physical model, all analytic derivations, and all scientific conclusions are the author's own. No AI system is credited as an author or contributor, and the author takes full responsibility for the final content.

\paragraph{Modifications relative to earlier drafts of this manuscript.} (1) The $e$-fold number is locked to $N=\ln(a_{\rm end}/a_*)$ with explicit matching assumptions; all tabulated $n_s$, $r$, $\lambda_0$ use exact potential slow-roll at that $N$ (not mixed attractor closed forms). (2) At $\xi=11.1$: $N=50$ $\Rightarrow$ $\lambda_0=6.70\times10^{-8}$, $r=0.00425$, $n_s=0.9616$; anomaly $T_{\rm reh}\sim10^9\,$GeV $\Rightarrow$ $N\simeq50.7$. Planck $2\sigma$ band $N\approx45$--$56$ has $r\leq0.0052$. (3) Appendix no longer promotes the incorrect $\lambda_0=48\pi^2\xi^2A_s/N^2$ formula. (4) $\Ztwopsi$ stability is conditional on an anomaly-free UV discrete gauge completion. (5) Dark energy is a calibrated frozen $V_c$; single-field residual quintessence is excluded by $m_\chi/H_0\sim10^{55}$. (6) A parallel topological-defect DM channel is listed. (7) Figures regenerated with the locked-$N$ convention (\texttt{scripts/lock\_n\_convention.py}). (8) The dark-matter production exponent is corrected from $e^{-\pi m_\psi/H_{\rm inf}}$ to the exact Bogoliubov result $e^{-2\pi m_\psi/H_{\rm inf}}$; the exponential-framework matching point is $g\simeq6.9\times10^{-5}$. (9) The gravitational pulse of channel (i) does not reheat the universe (radiation fraction scales as $a^{-1}$); reheating is attributed solely to the conformal-anomaly channel. (10) $K_{\rm end}=\epsilon_H V_{\rm end}/(3-\epsilon_H)\simeq0.199V_{\rm end}$; the Planck band becomes $N\approx45$--$56$ ($N_{\rm max}=55.6$). (11) $\Delta w_{\rm quant}\lesssim2\times10^{-121}$; the residual-condensate bound $\leq0.39$ is a constraint satisfied because $\Gamma t_0\gg1$. (12) The QCD $b_3$ and $\alpha_s(m_\chi)$ entering the anomaly are given explicitly. (13) Appendix D records the end-of-inflation numerical validation. (14) Abundance matching with the true mode-equation spectrum selects the light branch $g\simeq1.0\times10^{-7}$ ($m_\psi\simeq7.5\times10^{10}$ GeV) in the transition-only calculation; the heavy exponential-framework point overproduces at the model's own $T_{\rm reh}$. (15) A reheating error-band analysis (\texttt{scripts/treh\_error\_band.py}, Table~\ref{tab:trehband}) quantifies the $T_{\rm reh}$ dependence: over a factor-$10^{2}$ band the inflationary observables move by $\Delta n_s\simeq1.1\times10^{-3}$ and $\Delta r/r\simeq5.5\%$, whereas the dark-matter matching point and $m_\psi$ move by a factor $10$, and the light branch remains cold dark matter throughout. The numerical equivalence of $n_s(N)$ and $r(N)$ with $R^2$ inflation is stated explicitly, together with the exact relation $m_\psi/H_{\rm inf}=(g/\sqrt{\xi})(\MP/H_{\rm inf})$, the reason the dark-energy sector is necessarily a frozen constant rather than residual motion of $\Phi$, and a concrete lattice parameter set for the remaining non-perturbative cross-check of the dark-matter normalization. (16) The cosmological matching \eqref{eq:matchN} is evaluated with comoving-entropy conservation in the radiation era instead of the constant-$g_*$ scaling $a\propto\rho^{-1/4}$, and with $\rho_{\rm end}=K_{\rm end}+V_{\rm end}$ rather than $V_{\rm end}$; $\Omega_r$ is the Planck value $9.15\times10^{-5}$ rather than the earlier rounded $9\times10^{-5}$, which alone shifts $T_{\rm reh}^*$ by $1.2\%$. This raises $T_{\rm reh}^*$ by a factor $\simeq2.5$ \emph{relative to the constant-$g_*$ route with $V_{\rm end}$} (the combined effect of entropy matching, $\rho_{\rm end}=K_{\rm end}+V_{\rm end}$ and the $\Omega_r$ update; equivalently $\simeq0.30$ $e$-folds in $N$): the tabulated values become $T_{\rm reh}^*(50)=1.1\times10^{8}\,$GeV and $N_{\rm max}=55.6$, and the anomaly value $T_{\rm reh}\sim10^{9}\,$GeV maps to $N\simeq50.7$. The independent first-principles cross-check of \texttt{background\_and\_reheating.py} now agrees with the table to $0.1\%$; its $x_*$ relation was corrected to Eq.~\eqref{eq:Nint}, the form $e^{2x}-2x=2\beta^2N$ used earlier being wrong for this potential. (17) The two Sec.~XI consistency entries for dark-matter self-interaction and the Tremaine--Gunn bound are recomputed at the abundance-matched mass: $\sigma_{\psi\psi}/m_\psi\sim7\times10^{-70}\,{\rm cm}^2/{\rm g}$ and $Q\sim3\times10^{43}\,$GeV$^4$, replacing values that had been quoted at a wimpzilla-scale $m\sim10^{13}\,$GeV rather than at the $m_\psi\simeq7.5\times10^{10}\,$GeV that Sec.~V selects. Both are now produced by \texttt{scripts/order\_estimates.py} and checked by \texttt{scripts/verify\_numerics.py}. (18) The dark-energy discussion now confronts DESI DR2~\cite{DESI2025}, which prefers time-evolving dark energy over $\Lambda$CDM at $3.1\sigma$: the frozen $w_0=-1$ prediction is stated as a live exposure rather than as a future test, since a frozen $\chi$ and a constant $V_c$ leave no room for such an evolution. The former Sec.~VII is folded into Sec.~VI as the subsection \emph{Drift of the fundamental constants}, whose Ricci-driving bound it duplicated. (19) The absolute normalization is closed at the homogeneous level by the exact-background mode integration through the oscillating-condensate era (\texttt{scripts/psi\_mode\_oscillating.py}): the matched coupling moves to $g\simeq1.5\times10^{-7}$ ($m_\psi\simeq1.1\times10^{11}$ GeV), the earlier upper-bound reading of the transition-only value is reversed (the transition-shape systematic dominates and pushes $g$ upward), and the computation is validated by the convergence/unitarity/conformal battery of Appendix~D0.
