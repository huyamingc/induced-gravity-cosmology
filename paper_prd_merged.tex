...
\abstract{We construct a scalar-tensor cosmology in pure induced gravity: a real scalar $\Phi$ with $\Ztwo$ potential $V_J(\Phi)=\frac{\lambda_0}{4}(\Phi^2-\PhiV^2)^2+V_c$ and coupling $\xi\Phi^2R$ replaces the Einstein--Hilbert term, generating $\MP$ via $\vev{\Phi}=\MP/\sqrt{\xi}$. Conformal transformation yields a Starobinsky-type slow-roll plateau. With locked $N\equiv\ln(a_{\rm end}/a_*)$ and exact potential slow-roll, $\xi=11.1$ gives $N=50\Rightarrow\lambda_0\simeq6.70\times10^{-8}$, $r\simeq0.00425$, $n_s\simeq0.962$; anomaly $T_{\rm reh}\simeq10^9$ GeV maps to $N\simeq50.7$. Planck $2\sigma$ selects $N\approx45$--$56$ ($r\leq0.0052$; capped at $N_{\max}\simeq55.6$ by instantaneous reheating); LiteBIRD $r>0.01$ would exclude the model class. A Dirac fermion $\psi$ with $-g\Phi\bar\psi\psi$ is a cold-dark-matter \emph{candidate}: $m_\psi=g\PhiV$ shares the VEV; stability relies on an assumed anomaly-free discrete $\Ztwopsi$ plus the conformally vanishing Yukawa vertex. Matching $\Omega_{\rm DM}$ with the exact mode-equation spectrum on the exact homogeneous background (transition and oscillating-condensate era) selects $g\simeq1.5\times10^{-7}$ ($m_\psi\simeq1.1\times10^{11}$ GeV $\simeq6.6\times10^{-3}H_{\rm inf}$), cold kinematics ($\lambda_{\rm fs}\sim10^{-19}$ Mpc); absolute normalization is closed at the homogeneous level (convergence/unitarity/conformal battery), with transition-shape systematic ($\times1.6$) and a 3D preheating cross-check remaining. The potential minimum gives a frozen cosmological constant $w=-1$ (residuals $\leq10^{-10}$); dark energy is calibrated, not predicted ($V_c$ fixed by $\Omega_\Lambda=0.683$, $10^{110}$ hierarchy unexplained); confirmed DESI DR2 preference for $w_0\neq-1$ would exclude it and $\Lambda$CDM alike. Tree-level $\chi\to h\bar h$ and $\chi\to\psi\bar\psi$ close; reheating proceeds via conformal anomaly. SM embedding is RG-stable ($\Delta\xi\leq10^{-6}$); $w_0=-1$ is shared with $\Lambda$CDM; the sharp falsification handles remain inflationary.}
...
A single closed-form matching estimate, $N=61+\frac14\ln(V_{\rm end}/\MP^4)-\frac13\ln(V_{\rm end}^{1/4}/T_{\rm reh})$ (Liddle--Leach style~\cite{LiddleLeach2003}), remains a useful cross-check, but the physical window used here follows from the locked definition \eqref{eq:Ndef} and Planck $n_s$, Eq.~\eqref{eq:Nwindow}.
...
\label{eq:A5}
\end{equation}
\emph{Note.} The shortcut formula $\lambda_0\simeq48\pi^2\xi^2A_s/N^2$ ($4.90\times10^{-8}$ at $\xi=11.1$, $N=50$) omits the factor $(6+1/\xi)$ and is inconsistent with Eq.~\eqref{eq:A4}. The large-field analytic estimate \eqref{eq:A5} gives $\lambda_0\simeq7.46\times10^{-8}$; the \textbf{locked-$N$ exact slow-roll inversion} used throughout the main text (Eqs.~\eqref{eq:Nint}--\eqref{eq:obsPS}) gives $\lambda_0\simeq6.70\times10^{-8}$ at $N=50$ and $\simeq6.45\times10^{-8}$ at $N=51$. Table~\ref{tab:sens} and all fiducial scales quote the locked-$N$ exact values.
